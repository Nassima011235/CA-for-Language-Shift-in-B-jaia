...
This repository also contains code for computing and generating pre-images of finite patterns under the cellular automaton. Given a pattern, the algorithm searches for patterns whose image under the cellular automaton is equal to the specified pattern.

Whenever a pre-image exists, the corresponding pattern can be generated by the code. If no valid configuration satisfies the required local constraints, the code indicates that the pattern has no pre-image.
